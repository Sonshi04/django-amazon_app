...
def crawl(request):
    ...
    for page in range(1, 9):
        url += f"&price={min_price*100}-{max_price*100}&page={page}"
        print(url)
        doc = BeautifulSoup(requests.get(url, headers=HEADERS).content, "html.parser")
        product_list = doc.select(".s-card-container .a-section.a-spacing-base")
        #商品なければ終わり
        ...
        for product in product_list:
            ...
            product_url = "https://www.amazon.co.jp" + product.select_one(
                ".s-product-image-container .a-link-normal"
            ).get("href")
            # 価格存在確認
            if len(price) == 0:
                continue
